                ...
                <tbody class="table-group-divider" id="body-tabla">
                    ...
                    for ($i = $mes_inicio; $i <= $mes_fin; $i++) : ?>
                        <tr>
                            <td><?= $meses[$i] ?></td>
                            <td>
                                <a class="btn btn-danger rounded-0 text-uppercase" href="<?= URL_RAIZ ?>view/generar-reporte/docente/reporte-mes.php?mes-inicio=<?= date('Y') ?>-<?= ($i<10) ? '0'.$i : $i ?>-01&clase=<?= $resultados->id ?>"><i class="bi bi-download me-2"></i>Asistencias del Mes</a>
                            </td>
                        </tr>
                    <?php endfor; ?>
